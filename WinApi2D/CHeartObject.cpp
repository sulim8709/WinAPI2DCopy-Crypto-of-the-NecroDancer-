...
void CHeartObject::CreateBeatNode()
{
	...
	pBeatNode3->SetPos(fPoint((WINSIZEX / 8), WINSIZEY - 80.f));
	pBeatNode3->SetDir(fVec2(1, 0));
	pBeatNode3->SetName(L"BeatNode3_HeartBeat");

	CBeatNode* pBeatNode4 = new CBeatNode;
	pBeatNode4->SetPos(fPoint((WINSIZEX / 8) * 7, WINSIZEY - 80.f));
	pBeatNode4->SetDir(fVec2(-1, 0));
	pBeatNode4->SetName(L"BeatNode4_HeartBeat");

	// 3/4 지점
	CBeatNode* pBeatNode5 = new CBeatNode;
	pBeatNode5->SetPos(fPoint((WINSIZEX / 8) * 2, WINSIZEY - 80.f));
	pBeatNode5->SetDir(fVec2(1, 0));
	pBeatNode5->SetName(L"BeatNode5_HeartBeat");

	CBeatNode* pBeatNode6 = new CBeatNode;
	pBeatNode6->SetPos(fPoint((WINSIZEX / 8) * 6, WINSIZEY - 80.f));
	pBeatNode6->SetDir(fVec2(-1, 0));
	pBeatNode6->SetName(L"BeatNode6_HeartBeat");

	// 4/4 지점
	CBeatNode* pBeatNode7 = new CBeatNode;
	pBeatNode7->SetPos(fPoint((WINSIZEX / 8) * 3, WINSIZEY - 80.f));
	pBeatNode7->SetDir(fVec2(1, 0));
	pBeatNode7->SetName(L"BeatNode7_HeartBeat");

	CBeatNode* pBeatNode8 = new CBeatNode;
	pBeatNode8->SetPos(fPoint((WINSIZEX / 8) * 5, WINSIZEY - 80.f));
	pBeatNode8->SetDir(fVec2(-1, 0));
	pBeatNode8->SetName(L"BeatNode8_HeartBeat");

	// 5/4 지점
	CBeatNode* pBeatNode9 = new CBeatNode;
	pBeatNode9->SetPos(fPoint(-(WINSIZEX / 4 ), WINSIZEY - 80.f));
	pBeatNode9->SetDir(fVec2(1, 0));
	pBeatNode9->SetName(L"BeatNode7_HeartBeat");

	CBeatNode* pBeatNode10 = new CBeatNode;
	pBeatNode10->SetPos(fPoint((WINSIZEX / 8) * 10, WINSIZEY - 80.f));
	pBeatNode10->SetDir(fVec2(-1, 0));
	pBeatNode10->SetName(L"BeatNode8_HeartBeat");
	...
	CreateObj(pBeatNode9, GROUP_GAMEOBJ::NODE);
	CreateObj(pBeatNode10, GROUP_GAMEOBJ::NODE);*/
